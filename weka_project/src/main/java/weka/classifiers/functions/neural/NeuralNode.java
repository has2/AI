...
//   m_weights[0] = m_random.nextDouble() * .1 - .05;
    ...
    //after
    //초기 weight sum이 0이상의 값을 가지도록 초기 bias 값을 0.1의 값을 가지도록 초기화 
    m_weights[0] = .1;
  ...
  protected boolean connectInput(NeuralConnection i, int n) {
    ...
    //after
    //초기 weight sum이 0이상의 값을 가지도록 초기 weight 값을 0~0.05의 값을 가지도록 초기화 
    m_weights[m_numInputs] = m_random.nextDouble() * .05;
